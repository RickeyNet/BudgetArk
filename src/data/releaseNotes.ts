...
export const RELEASE_NOTES: readonly ReleaseNote[] = [
  {
    ...
    highlights: [
      ...
      "The Data card on the Profile tab is down to four rows. Export and Import each open a short menu - encrypted backup or spreadsheet on one side, backup file, pasted backup text, spreadsheet or bank statement on the other - instead of six look-alike rows competing for attention. Every option does exactly what it did before.",
      "The feature tour introduces the seven 1.9.0 themes on one slide instead of three, so replaying it from the Profile tab is quicker. Nothing else about the tour changes, and if you already saw the original slides you won't see this one as new.",
      "Bank balances refresh on every launch. The automatic bank sync only armed itself once you opened the Profile tab, so a day spent on the Bridge or Budget tabs never pulled a fresh balance. It now arms at startup, whichever tab you land on, and still keeps to one automatic pull per six hours.",
      "No more double-counted card charges. A pending charge that later posted with a tip added, a fuel hold replaced by the real total, or a slow weekend settlement used to arrive as a second row in the Review Inbox. The inbox now recognises the posted charge as the same purchase when the amount is within 30% and it settles within a week, and if you had already approved the pending row, the entry's amount is corrected to what actually settled (unless you edited it yourself). Pending rows your bank stops reporting retire on their own after three days instead of lingering.",
    ],
  },
  {
    version: "1.10.4",
  ...
  },
  {
    ...
    highlights: [
      ...
      "Review Inbox: your debts now appear in the 'Applies to bill' list. Open a card or loan payment that a bank import brought in and tap the debt - the amount is logged as a payment on the Debts tab (balance and history), the Budget counts it under Debt Payments, and no separate expense is filed, so the payment is never counted twice. Tick 'Always do this' and the merchant rule remembers the debt: future payments to that merchant (and any already waiting in the inbox) are logged on it without stopping for review, even when the bank text looks like a transfer. Rules can be pointed at a debt later from the inbox's Rules screen too. Paid-off debts stay out of the list. And it never double counts against the due-day reminder: if you already confirmed 'paid the minimum today' (or logged the payment on the Debts tab) and the bank's copy of that same payment arrives a few days later, the row is matched to the payment already on file and cleared without touching the balance - the inbox tells you so. A nearby hand-logged payment for a different amount is left for you to decide.",
      "The Add / Edit Entry sheet's DAY picker is now a real calendar: weekday headers (Sun to Sat) with each date in its own column and today outlined, so you can tell Tuesday the 8th from Wednesday the 9th at a glance instead of counting along a strip of numbers. It follows the month you picked above it.",
