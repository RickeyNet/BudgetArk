...
describe("demo data - app startup reads and math", () => {
  ...
  it("every startup storage read returns a sane shape", async () => {
    ...
    // Structural invariants rather than exact counts: the fixture is
    // regenerated by scripts/generate-demo-data.mjs at test run time, so
    // pinning e.g. "exactly 4 debts" or "exactly 40 net-worth points" makes
    // the test go red on a cosmetic generator tweak instead of a real bug.
    ...
    for (const debt of debts) {
      ...
      expect(debt.rate).toBeGreaterThanOrEqual(0);
    ...
    // Enough points to draw a trend line; every point internally consistent.
    expect(netWorthSnapshots.length).toBeGreaterThan(1);
    for (const snap of netWorthSnapshots) {
      ...
      expect(Number.isFinite(snap.totalDebt)).toBe(true);
      expect(snap.netWorth).toBeCloseTo(snap.totalAssets - snap.totalDebt, 2);
    }

    const monthStartBalances = await getMonthStartBalances();
    const balanceMonthKeys = Object.keys(monthStartBalances);
    expect(balanceMonthKeys.length).toBeGreaterThan(0);
    for (const key of balanceMonthKeys) {
      expect(key).toMatch(/^\d{4}-(0[1-9]|1[0-2])$/);
      expect(Number.isFinite(monthStartBalances[key].balance)).toBe(true);
    }
