...
## v1.10.1 - Bank Account Categories & Wizard Freeze Fix (2026-09-01)

**OTA-shippable.** Pure JS; `runtimeVersion` stays 1.10.0, so every 1.10.0 store build receives this over the air.

- **Any Bridge category is a bank-balance target.** `MAPPABLE_ASSET_CATEGORIES` now lists all six categories (retirement and investment included, picker order checking → savings → retirement → investment → hsa → other) and both the wizard's mapping step and the Connections manager's editor list every Bridge account in the "Balance updates" picker instead of filtering out pure-holdings ones. `applyBalances` in `connectionsSyncService` and the seed path in `updateLinkPreferences` drop their `categoryIsPureHoldings` skip: a synced 401k / brokerage balance lands on `AssetAccount.balance`, which `buildHoldingsCategoryData` already counted and the Bridge already rendered as the broker's Cash line (`showCashRow = hasCash || balance > 0`), so no valuation or UI change was needed - the "+ New account" form gains a hint that tickers added on the Bridge count on top of the synced balance. Sync test updated to assert the write.
- **Category guessed from the account name.** New pure `services/connections/assetCategoryHint.ts` (`suggestAssetCategory`, unit-tested): word-bounded keyword rules, specific first (401k/403b/IRA/Roth/pension → retirement, HSA, brokerage/529 → investment, savings/money market/CD → savings, checking/chequing/cash management → checking), fallback checking. Both "+ New account" forms preselect it when the form opens; previously everything defaulted to Checking, which is how synced accounts were miscategorized. SimpleFIN reports no account type and Teller's subtype is lost in normalization, so the name is the only signal.
- **Fixed: wizard "Connected" screen freezing the app on Done.** Third fix in this family. `ConnectionsSection` now routes Done and Cancel through one `closeWizard(afterDismiss)`: the callback (post-setup sync kick, or the manager refresh on Cancel) runs from `Modal.onDismiss` on iOS and from a 700 ms timer (`ANDROID_MODAL_TEARDOWN_MS`) on Android - `InteractionManager.runAfterInteractions` fired as soon as the Done button's press animation ended, i.e. inside the native dialog teardown, and a fast-failing first sync's `refreshConnections()` then re-rendered the still-visible Connections manager mid-teardown (the Android UI-thread wedge from v1.9.x). The queued callback is replaced rather than stacked (a repeat Done tap runs one sync) and the timer is cleared on unmount. Device verification required on both platforms.
- **Review Inbox no longer re-offers what a partner phone already reviewed.** Two gaps, both hit when a second phone connects to the same SimpleFIN bridge after pairing. (1) `planIngest`'s dedupe only sees transactions as they are FETCHED, so a row already in the inbox stayed there when the partner's approved entry (carrying `externalTxId`) arrived over sync afterwards. New pure `planInboxReconciliation` in `services/connections/ingest.ts` retires inbox rows whose key is in the ledger, on an entry (recorded as "approved" with the entry id), or whose pending twin was decided under a reissued id (ledger alias, one row per decision via the shared `buildDecidedTwinFinder`); `reviewInboxService.reconcileInboxWithDecisions` is its storage shell (ledger first, inbox removal last), run best-effort at the start of every bank-sync pass and inside `applyIncomingDiff` whenever a diff brought entries or dismissals. `planIngest` also aliases a pending inbox twin when a synced decision names the posted id, so it can't sit as "pending" forever. (2) Dismissed decisions lived only in the deciding phone's ledger ("may reappear once - accepted v1 degradation"). New optional `SyncDiff.dismissedTransactions?: Record<identityKey, IngestLedgerEntry>` carries the ledger's DISMISSED entries (never approvals - their entry already carries the key, and a decision behind a 🔒 private entry must not leak): `selectSyncableDismissals` picks them incrementally by `at`, with a one-time full send gated by a second backfill flag (`@budgetark_sync_backfill_dismissals_v1`, stamped alongside the v1 flag by `markBackfillSyncDone`) so already-paired couples send their pre-existing dismissals once. Receiver: `isIngestIdentityKey` / `isIngestLedgerEntryRecord` (dismissed only, safe text, ≤ 400 chars) per key/value, `MAX_SYNCED_DISMISSALS` = 5000 cap, whole diff rejected on any failure; `reviewInboxStorage.mergeLedgerFromSync` merges key-wise, strictly-newer `at` wins, then the inbox reconciliation runs. Older peers ignore the field and their diffs still apply. `ConnectionsProvider` now re-reads on `dataChangeNotifier` events so the inbox badge drops when a partner sync retires rows. Privacy note: a dismissed pending item's fingerprint (`account|amount|day`) travels with it so the partner recognizes the posted twin - the only per-transaction detail the field carries; the privacy policy's Partner Sync section now says so. Tests: planner, selector, ledger merge, validators, diff-engine send/validate/merge/reconcile, and a storage-backed reconcile in `reviewInboxService.test.ts`.
- **Onboarding template step: inputs stay above the keyboard.** The onboarding ScrollView had no keyboard handling, so the take-home pay / rent fields on the template step (and the display-name field) sat hidden while typing. `OnboardingScreen` now reuses the screen-level pattern from Profile/Charts: `automaticallyAdjustKeyboardInsets` on iOS, `KeyboardAvoidingView` + `useAndroidKeyboardInputScroll` on Android.

## v1.10.0 - Sturdier Sync, Safer Data (2026-08-28)

**Store build required.** `runtimeVersion` 1.9.0 -> 1.10.0: the new app icon, adaptive icon, and splash are baked into the binary, so this version ships as a new EAS build on both platforms (the code itself is pure JS and would otherwise have been OTA-eligible; 1.9.0 installs will not receive it over the air). Minor bump rather than patch: no breaking change anywhere (protocol, HMAC, and envelope untouched; the only sync addition is an optional field older peers ignore; 1.9.x exports still import), but bank-mirrored credit-card balances are a new capability and the data-integrity fixes below change behaviour users will notice - together they deserve more than a "bug fixes" line.

- **New app icon.** The ark-and-sunburst mark replaces the old logo: `assets/icon.png` (iOS, full bleed), `assets/adaptive-icon.png` (Android foreground, art at 80% so the rays survive the launcher crop), `assets/splash-icon.png` (transparent ink with a radial fade), `assets/favicon.png`; icon/splash `backgroundColor` now `#FFF1DC` to match the logo's cream. Old `ArkLogo.png`/`applogo.png` source art removed. Play Console store-listing icon is a separate 512 upload (`assets/BudgetArklogonew.png`).
...
- **Onboarding quick-start templates.** `data/quickStartTemplates.ts` holds four templates as category allocations (% of take-home pay) with `zeroBased` / `suggestsArkSetup` flags; pure `utils/quickStart.ts` `buildQuickStartSeed(template, { monthKey, now, income, housing })` returns current-month `CategoryBudgetLimit[]` (rounded to 5, the typed housing payment overriding Housing's share, zero-based pushing the rounding remainder into Savings so limits total pay exactly) plus recurring `Take-home pay` (Salary) and `Rent / mortgage` (Housing) `NewBudgetEntryInput`s when those amounts were given; `parseQuickStartAmount` is the lenient input parse. `OnboardingScreen` step 4 of 6 collects the choice and the two optional numbers; `seedQuickStart` runs after `completeOnboarding` succeeds (a seed failure alerts and never re-runs onboarding) via `addBudgetEntries` + `saveCategoryBudgetLimits`. No custom categories are created.
- **Tracking Reminders offered during onboarding.** `OnboardingScreen` gains a `reminders` step between the template step and the display name (steps renumbered 1-6). "Turn on reminders" calls `ensureTrackingReminderPermissions()` (the OS prompt), and only when granted writes `{ ...DEFAULT_TRACKING_REMINDER_SETTINGS, enabled: true }` and reschedules; a declined prompt leaves the stored setting off (still opt-in in storage - nothing is enabled behind the user's back) and explains the Settings path. Copy states the rule-11 promise: no amounts, balances, accounts, or bills in a notification, and no payment-due alerts. Existing installs get the same question once as a self-retiring `TrackingReminderOfferCard` at the top of the Budget tab: shown only while `shouldOfferTrackingReminders()` (`utils/trackingReminderOffer.ts`, pure) holds - reminders off, no settings record ever written (`hasStoredTrackingReminderSettings`), and no answer recorded under the new device-local `@budgetark_tracking_reminder_offer` key, which both the card and the onboarding step write on any answer.
- **Inline category creation + hideable built-ins.** `CategoryPillPicker` gains `allowCreate` (a dashed "+ New" pill expanding an inline name/icon/bucket form that calls `useCustomCategories().add` and selects the result - inline on purpose, never a second Modal on a sheet); wired in `BudgetEntryModal` and `ReviewInboxModal`. New pure `utils/categoryVisibility.ts` (`SELECTABLE_BUILT_IN_CATEGORIES` - the canonical picker list, re-exported from the picker for older importers - `PROTECTED_BUILT_IN_CATEGORIES` = ["Other"], `canHideBuiltInCategory`, fail-closed `parseHiddenBuiltInCategories`, `visibleBuiltInCategories`) and `storage/hiddenCategoriesStorage.ts` (`@budgetark_hidden_builtin_categories`, device-local, atomic `updateItem`, not synced or exported - `BUDGET_CATEGORIES` itself is untouched so validators, sync and imports are unchanged). `CustomCategoriesProvider` exposes `hiddenBuiltIns` / `visibleBuiltIns` / `setBuiltInHidden`; the picker, BudgetScreen's bulk-move options and `BudgetLimitsModal` read `visibleBuiltIns` (the Limits sheet keeps a hidden built-in's row while it still carries a limit so a save can't silently drop it). `ManageCategoriesModal` gains a built-in section with Hide / Restore.
- **Bridge tracking strip.** New pure `utils/trackingStrip.ts`: `buildTrackingStrip({ entries, limits, monthKey, now })` -> month-to-date expense spend (via `entriesForMonth`, so a fulfilled bill's actual replaces its estimate exactly as on Budget), the sum of live positive limits (null when none), whole days since the newest non-recurring entry's `createdAt` (null when nothing logged), and the newest N logged rows with the bill label from `describeFulfillment`; `describeDaysSince` phrases the habit line. New `TrackingStripCard` mounted in BridgeScreen's list header under the keep-alive banner; it reads the month's limits itself (`getCategoryBudgetLimits`, re-read when the host's entries change) and hands off through Budget's existing route params (`quickAdd: {}` opens the Add sheet, `searchEntryId` opens an entry) so Budget stays the single owner of those modals.
- **Entry form memory.** New pure `utils/entryMemory.ts`: `buildDescriptionMemory(entries)` (one row per type + normalized description, latest casing/category, use count, recency by `createdAt`), `suggestDescriptions(memory, { type, category, query })` (no query: the category's recent names; query: prefix hits then substring hits across every category, never the fully typed name) and `categoryForDescription(memory, type, text)` (exact match). `BudgetEntryModal` renders the chips under each add-mode description input (tap = fill + switch category), switches the category on an exact typed match, and gains "Save + another" - `onAdd(entries, { keepOpen })` keeps the sheet up with the same category/date/tags and fresh amount lines; BudgetScreen honours `keepOpen` by not closing.
- **Recurring bill detection in the Review Inbox.** New pure `utils/recurringBillDetection.ts`: `detectRecurringBill(item, entries)` returns a suggestion (label, average of the charges, median posting day capped at 28, qualifying months) only when the charge is an outflow with a merchant key, no `suggestedRecurringId`, no live recurring expense already carries that `merchant`, nothing from the merchant this month yet, and each of the `RECURRING_BILL_MIN_MONTHS - 1` preceding months holds exactly one unlinked expense from it (two in a month = a store). `ReviewInboxModal` shows the "Looks like a monthly bill" card; "Make it a recurring bill" writes the bill (draft name/category, `merchant` stamped so it is never re-offered, start = this month), reloads via `onChanged`, preselects it in "Applies to bill" and ticks "Always do this" so the merchant rule learns `recurringEntryId`.
- **Limits sheet.** New pure `utils/limitsSheet.ts`: `resolveLimitsForMonth(history, monthKey)` mirrors storage's exact-else-latest-earlier rule, `buildLimitSheetRows` joins current/last-month limits with this month's spend and the mean over the preceding `LIMIT_LOOKBACK_MONTHS` months that had any expense data, `suggestLimitFromAverage` (up to the nearest 10, floor 10), `limitsFromDrafts` (blank/zero = removal, unchanged amounts keep `updatedAt` for LWW). New `BudgetLimitsModal` (SheetModal) opened from the Spending card header's "Limits ›" (`SpendingCard.onOpenLimits`); saves through `saveCategoryBudgetLimits` so removals tombstone for sync exactly like the long-press editor.
- **Occasional Tip Jar note after wins.** New pure `utils/tipJarNudge.ts`: `recordWinForNudge(state, now)` counts every win and grants a nudge only when `TIP_NUDGE_WINS_BETWEEN` (4) wins have passed since the last one AND `TIP_NUDGE_MIN_DAYS_BETWEEN` (7) days have elapsed (a clock that went backwards reads as too soon); `tipNudgeCopyFor(win)` supplies the brief copy per kind (`debt-payment | debt-payoff | bill-paid`). `storage/tipJarNudgeStorage.ts` persists `{ winsSinceNudge, lastNudgeAt, totalWins }` under `@budgetark_tip_nudge` via `updateItem` (device-local, not synced/exported, records nothing about actual tips). New `tipjar/TipJarProvider.tsx` at the app root owns one on-demand `TipJarModal` mount plus a floating toast; `useTipJar()` exposes `noteWin` / `showNudgeToast` / `openTipJar` (the latter defers past the caller's Modal dismiss). Wins wired: DebtCard inline payment (toast), payoff + due-prompt payments (inline `TipJarNudgeCard` inside the two celebration modals, in both DebtTrackerScreen and DebtDueReminderHost), Review Inbox approvals filed against a bill (inline card in the sheet), and Add/Edit Entry saves that link an actual to a bill (toast). Deliberately no off switch (a stale `enabled:false` from a pre-release build is ignored on read) - the cadence is the courtesy. Never a stacked Modal.
...
- **Tests:** `billFulfillment.test.ts` (21) plus fulfilment cases in `billCalendar`, `expenseCategoryRows`, `reviewInboxService` (validation, suggestion fallback, auto-approve, rule tri-state), `ingest`, `spreadsheetExport.csv`, `spreadsheetRoundTrip`. Suite: 1,978 tests.
...
- **Multi-person expenses.** `BudgetEntry.personIds?: string[]` (validated: ≤ `MAX_PEOPLE` safe ids) alongside `personId`, which stays the FIRST assignee so older peers, JSON restores and v5/v6 spreadsheets still see one person. `utils/entryPeople.ts` is the single reconciliation point: `entryPersonIds` (the single field wins when the list no longer contains it - an older peer edited the assignment) and `personAssignmentFields` (one person is still stored the pre-multi way, so nothing changes for existing data). `BudgetEntryModal`'s people pills are multi-select (each dangling id gets its own untag pill); `computePersonMonthSpending` and `computePersonReport` split shared amounts evenly (`personShare`) so totals still sum to real spend; the Budget entry badge lists every name; `ManagePeopleModal` counts a shared entry for each person. Spreadsheet schema v7 adds a `PersonIds` (";"-joined, round-trip) column and `Person` lists every name.
- **Multi-person Review Inbox and merchant rules.** `MerchantRule.personIds?: string[]` and `PendingTransaction.suggestedPersonIds?: string[]` mirror the entry pairing exactly (`personId`/`suggestedPersonId` stay the FIRST id; readers reconcile through `entryPersonIds`, writers through `personAssignmentFields`, one person is still stored the pre-multi way). `ApproveOptions.personIds` / `ChangeRuleOptions.personIds` replace the single-id options (`null` or `[]` = explicitly nobody, omit = keep/suggest). `merchant.suggestedPeopleFor` is the one derivation (rule people, else the card's person) shared by `planIngest` and `replanInboxForRules`; the auto-approve sweep applies every person on the rule; `clearAssigneesFromMerchantRules` drops only the deleted member and re-points `personId` at the next one. New `MultiTagPillPicker` (in `TagPillPicker.tsx`) is the shared multi-select pill row - `ReviewInboxModal` and `MerchantRulesModal` use it; the rule summary lists every name. Both stores are device-local (never synced/exported), so no protocol or migration impact. Tests: multi-person approve/remember/auto-approve, replan, ingest suggestion, rule-update payload, and delete cascade.
...
- **Export My Data shares a file, not Intent text.** `shareExportMessage` used `Share.share({ message })`, which on Android puts the whole backup into an Intent extra bounded by the ~1 MB Binder transaction limit; past it the chooser silently never launched (`!!! FAILED BINDER TRANSACTION !!!`) while RN still resolved `sharedAction`, so the app recorded a backup that never happened and stalled. Backups grown by bank-synced entries, holdings, snapshots and the collections added to the payload this release crossed that line. The export is now written to a temp file (`Paths.cache` on Android, `Paths.document` on iOS - same as `spreadsheetExport`) and handed to expo-sharing through `shareLocalFileThenDelete`, so there is no ceiling and the plaintext file is removed when the sheet closes or the share throws. Plain exports are `budgetark-backup-<stamp>.json` (`application/json`), encrypted envelopes `.txt` (`text/plain`); both are on the import picker's accept list. `recordBackup` now stamps on sheet dismissal, which is what the Android path effectively did before. `exportData.test.ts` drops the `Share` mock entirely (a re-import fails loudly) and gains a file-share suite incl. a 3 MB payload; this was a native transport limit the pure-logic Jest rig could not see - device verification still required.
