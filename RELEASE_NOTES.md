...
- **Tests:** 96 -> 122 suites, 1,388 -> 1,902 tests (incl. the new `debtBalances.test.ts` - magnitude/rounding, toggle default, high-water mark, link/debt gating - and six sync-orchestrator cases for the merged balance+stamp write); coverage ratchet raised to L80/S79/B76/F75 (measured L81.8/S80.9/B77.8/F76.3) with `src/hooks` and `src/notifications` now collected. New suites for the previously untested stores/services (budget, tombstones, review inbox, connection secrets, sync + inbox services, notification rule-11 guard, release notes), thin branches in debtStorage/encryptedStorage/importData/spreadsheets/diffEngine/validators/calculations, typed fixtures in `src/__tests__/fixtures.ts`, deterministic clocks, and a rewritten `docs/testing.md`.
