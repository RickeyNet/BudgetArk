...
## v1.9.0 - Bank Connections (2026-07-08)

**NOT OTA-shippable.** `react-native-webview` (13.16.0) is a new native dependency (Teller Connect enrollment), so `runtimeVersion` bumps 1.4.14 → 1.9.0 and this release requires a new dev-client/EAS build on both platforms. Existing runtimes never see this update over the air. SimpleFIN and Schwab have no native needs of their own - only the WebView does - but they ship in the same build. No server-side/Worker changes: the entire feature is device-to-provider by design.

### Bring-your-own-API bank syncing (`src/services/connections/`)

- **Architecture: the user brings their own credentials; BudgetArk operates no aggregator.** Three providers, each normalized into the same `NormalizedAccount`/`NormalizedTransaction` shapes (`types.ts`) with a shared error taxonomy (`auth-expired | invalid-credentials | rate-limited | network | provider-error`). All clients follow the quotesService contract: never throw, `AbortController` timeouts (15s), typed outcome unions.
...
- **Schwab Trader API (`schwabParser.ts`, `schwabTokenState.ts`, `schwabClient.ts`).** User registers their own developer app (key/secret, callback `https://127.0.0.1`). OAuth is a paste-back flow: `Linking.openURL` to the authorize URL, the browser dead-ends on the user's registered 127.0.0.1 callback, and the user pastes the full address back; `extractAuthCode` tolerates whole URLs, extra params, whitespace, and decodes `%40` exactly once. `planTokenAction` is a pure state machine over the token set: live access token → use; expired + live refresh token → refresh; refresh token past 7 days minus a 10-minute margin (or a refresh-grant 400/401) → `reauth-required`, surfaced in the UI as a Reconnect banner. `fetchSchwabData` returns refreshed tokens as `tokenPatch` even when a later request fails, and the orchestrator persists it unconditionally - dropping a rotated refresh token would strand the connection. Endpoint/field names (accountNumbers hash map, `netAmount` sign, `activityId`) are commented verify-against-current-docs; parsers isolate the blast radius.
...
- **Secrets live in `connectionSecretsStorage.ts`, NOT raw SecureStore:** Schwab/Teller blobs exceed Android SecureStore's ~2KB soft limit, and encryptedStorage gives the same at-rest guarantee with per-key write serialization. The secrets union (`SimplefinSecrets | SchwabSecrets | TellerSecrets`) is typed in the storage module so it never leaks into sync/export type surfaces.
...
- **`AddConnectionModal`** - one modal, an internal step machine (`provider → simplefinToken | schwabKeys→schwabRedirect | tellerSetup→tellerEnroll → mapAccounts → done`), inline errors, re-auth mode via `reauthConnectionId` (jumps straight to Schwab OAuth with stored keys, skips mapping). The mapping step offers per-account import toggles and AssetAccount pills including an inline "create new account" mini-form; balance targets exclude pure-holdings categories.
...
- **Tests: 89 new** across 10 suites (simplefin/schwab/teller parsers, token state machine matrix, ingest dedup incl. id-instability aliasing, merchant normalization, cooldown/window math, HTTP/1.1 codec incl. chunked bodies, validator lockstep, export exclusion, spreadsheet round-trip). Full suite: 610 passing.
