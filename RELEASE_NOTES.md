...
**NOT OTA-shippable.** `react-native-webview` (13.16.1) is a new native dependency (Teller Connect enrollment), so `runtimeVersion` bumps 1.4.14 → 1.9.0 and this release requires a new dev-client/EAS build on both platforms. Existing runtimes never see this update over the air. SimpleFIN has no native needs of its own - only the WebView does - but both providers ship in the same build. This release also carries the Expo SDK 57 / RN 0.86 / TS 6.0 upgrade (see "Follow-ups" below), which is itself build-affecting. No server-side/Worker changes: the entire feature is device-to-provider by design.
