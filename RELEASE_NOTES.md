...
**NOT OTA-shippable.** `react-native-webview` (13.16.1) is a new native dependency (Teller Connect enrollment), so `runtimeVersion` bumps 1.4.14 → 1.9.0 and this release requires a new dev-client/EAS build on both platforms. Later additions on this branch - `expo-iap` (Tip Jar), `react-native-quick-crypto` (native crypto), and `expo-notifications` (Tracking Reminders, section below) - are also native and ride the same build. Existing runtimes never see this update over the air. SimpleFIN has no native needs of its own - only the WebView does - but both providers ship in the same build. This release also carries the Expo SDK 57 / RN 0.86 / TS 6.0 upgrade (see "Follow-ups" below), which is itself build-affecting. No server-side/Worker changes: the entire feature is device-to-provider by design.

### Bring-your-own-API bank syncing (`src/services/connections/`)

- **Architecture: the user brings their own credentials; BudgetArk operates no aggregator.** Two providers (SimpleFIN, Teller), each normalized into the same `NormalizedAccount`/`NormalizedTransaction` shapes (`types.ts`) with a shared error taxonomy (`auth-expired | invalid-credentials | rate-limited | network | provider-error`). All clients follow the quotesService contract: never throw, `AbortController` timeouts (15s), typed outcome unions.
- **SimpleFIN Bridge (`simplefinParser.ts`, `simplefinClient.ts`).** Pasted setup token = base64 claim URL; POSTing it once yields an access URL with embedded basic-auth credentials (re-split into an explicit `Authorization` header - RN fetch doesn't reliably send URL-embedded auth). `GET {base}/accounts?start-date&pending=1`. Parser handles the protocol's quirks: money as decimal strings, dates as epoch seconds, pending rows with `posted: 0` falling back to `transacted_at`; malformed rows drop individually, never fatally. 403 on claim = "token already claimed" (they're single-use).
- **Schwab connector: built, then cut before release (2026-07-10).** A full Schwab Trader API integration (client, parser, OAuth paste-back flow, token-lifecycle state machine) shipped on the branch but was removed before any build: it was the narrowest-audience, highest-friction provider (own developer-app registration, 7-day refresh-token re-approval cycle) and the most complex of the three. The removal deleted the Schwab client/parser/token-state and their tests, dropped `"schwab"` from `BankProvider` and all consumers, removed `SchwabSecrets` from secrets storage, and removed the in-place re-auth flow (only Schwab's 7-day cycle needed it - a needs-reauth connection now advises remove-and-re-add). The code survives in git history if it's ever wanted back.
- **Teller (`http1.ts`, `tellerMtlsClient.ts`, `tellerParser.ts`, `tellerClient.ts`, `TellerConnectModal.tsx`).** Teller mandates mutual TLS for all real-data calls - RN fetch can't present a client identity, but `react-native-tcp-socket` (already shipped for LAN sync) accepts PEM `cert`/`key` in its TLSSocket options on both platforms (Android builds a KeyManagerFactory from the PEMs; iOS a SecIdentity), so **no new native module was written**. Requests run as hand-rolled HTTP/1.1 over that socket: `http1.ts` is the pure codec (request serialization, status/header parsing, content-length and chunked bodies; `Connection: close`, one exchange per socket). The user imports `certificate.pem`/`private_key.pem` from their teller.zip via the existing document picker and enters their application id; enrollment runs Teller Connect (their CDN's `connect.js`) in a WebView with `postMessage` handoff of `{accessToken, enrollmentId}` - the token never leaves the device. Access tokens are stored per enrollment; auth is Basic with the token as username. Fetch-window filtering is client-side (Teller returns newest-first; the ingest planner dedupes anyway).

### Data model & dedup (`src/types/index.ts`, `src/services/connections/ingest.ts`)

- **`BudgetEntry` gains optional provenance:** `source?: "bank"`, `externalTxId?` (the global dedup identity `${provider}:${externalAccountId}:${providerTxId}`, oversized provider ids SHA-256'd), and `merchant?` (normalized key). Deliberately no `connectionId` - connection ids are per-device and meaningless after P2P sync; `externalTxId` carries everything dedup needs and rides sync/export like any other entry field.
- **New per-device types:** `BankConnection` (non-secret metadata: status, lastSyncedAt/lastAttemptAt, error fields), `ExternalAccountLink` (provider account → AssetAccount mapping + per-account import/balance toggles; raw provider balance kept for display), `PendingTransaction` (inbox item; `id` IS the identity key), `MerchantRule` (unique on merchantKey), `IngestLedgerEntry`/`IngestLedger`, `ConnectionsSettings` (disclosure flag).
- **`planIngest` is the single guardian against duplicates** (pure, injectable clock). Per fetched tx: skip unlinked/import-off accounts and zero amounts → skip if in the ingest ledger (dismissed stays dismissed) → skip if `externalTxId` exists on ANY budget entry incl. tombstones (covers partner-approved entries arriving via P2P sync and restored backups) → update matching inbox rows (pending→posted, amount/description drift) → pending→posted **id-instability fallback**: a posted tx with an unknown id matches its pending twin by fingerprint (account + exact amount + ±4 days), migrating the inbox row to the new id and aliasing the old key in the ledger; decisions made while pending are honored via a stored `pendingFingerprint`. New rows get sign-derived type (negative = expense), a normalized merchant key, a rule-suggested category, and a `transferLikely` flag (description regex OR an opposite-signed same-amount tx on another linked account within ±3 days in the same batch) - flag only, never auto-dropped.
- **`normalizeMerchant`/`matchMerchantRule` (`merchant.ts`):** uppercase, punctuation-collapsed, noise tokens dropped (store `#1234`, dates, ≥10-char refs), 40-char cap; exact match beats longest-prefix (≥6 chars both sides, so "SQ" can't match everything).

### Storage & privacy boundary (`src/storage/connection*`, `reviewInboxStorage.ts`, `merchantRulesStorage.ts`)

- **Six new per-device stores, none tombstoned (unsynced data hard-deletes):** connections, secrets, account links, review inbox (newest-500 cap), ingest ledger (120-day TTL prune - far beyond any fetch window), merchant rules, and the disclosure settings. All in `encryptedStorage` (AES-256+HMAC, master key in Keychain/Keystore).
- **Secrets live in `connectionSecretsStorage.ts`, NOT raw SecureStore:** Teller cert/key blobs exceed Android SecureStore's ~2KB soft limit, and encryptedStorage gives the same at-rest guarantee with per-key write serialization. The secrets union (`SimplefinSecrets | TellerSecrets`) is typed in the storage module so it never leaks into sync/export type surfaces.
- **Nothing connection-related syncs or exports.** `SyncDiff` untouched (documented in `src/sync/types.ts`: merchant rules are the v2 sync candidate, needing a tombstone first). `exportData.ts` documents the exclusion and `exportData.test.ts` enforces it - a regression test asserts no connection/secret/inbox/rule keys in the export payload. Deleting a connection cascades to secrets/links/unreviewed inbox but **keeps the ledger** so reconnecting the same institution can't re-offer decided transactions. Ledger loss on reinstall degrades gracefully: approved txs still dedup via synced `externalTxId`; dismissed ones may reappear once.

### Sync orchestration (`connectionsSyncService.ts`, `syncGate.ts`, `reviewInboxService.ts`)

- **Cooldowns respect SimpleFIN's ~24 req/day budget:** auto-sync at most every 6h per connection, manual every 15min, gated on `lastAttemptAt` which is stamped BEFORE fetching so failing providers are never hammered. Fetch window = last success minus 7 overlap days (first sync: 30-day backfill); the overlap re-observes pending transactions until they post.
- **Foreground trigger:** `startConnectionsMonitoring()` registers an AppState "active" listener (idempotent, wired from ProfileScreen next to the existing LAN-sync `startMonitoring`). Module-level in-flight guard collapses concurrent passes. Balance application clamps at ≥0 (`isAssetAccountItem` rejects negatives on the sync receive path - raw value kept on the link for display), skips pure-holdings categories, and skips unchanged balances to avoid `updatedAt` churn spamming P2P diffs.
- **Approval write order is deliberate** (`approvePendingTransaction`): BudgetEntry via the existing `addBudgetEntry` → ledger → inbox removal → optional merchant rule. A crash mid-sequence leaves at worst a stale inbox row that dedup can't resurrect. Bulk dismiss mirrors it.

### UI (`ProfileScreen`, `BudgetScreen`, five new components)

- **Profile gains a CONNECTIONS section** (between DATA and SETTINGS): a state-aware Bank Connections row (count / "Needs attention" in warning color) gated on a first-use disclosure modal (copy in `src/data/connectionsDisclosure.ts`, holdingsDisclosure pattern), plus a Review Inbox row that navigates `Budget` with a new `openInbox` route param.
- **`AddConnectionModal`** - one modal, an internal step machine (`provider → simplefinToken | tellerSetup→tellerEnroll → mapAccounts → done`), inline errors. The mapping step offers per-account import toggles and AssetAccount pills including an inline "create new account" mini-form; balance targets exclude pure-holdings categories.
- **`ConnectionsModal`** - list ⇄ detail manager: status lines, needs-reauth banner + Reconnect, per-connection Sync Now, linked-account summary, Remove with confirm (credentials deleted, approved entries stay, unreviewed items discarded).
- **`ReviewInboxModal`** - SectionList grouped by posted date with a separate "Likely transfers" section; rows expand into a category picker + "always use this category" checkbox; Approve/Skip per row; a bulk bar approves everything rule-suggested. Approvals refresh BudgetScreen through a new `reloadAfterInboxChange` (entries re-read + net-worth/monthly-review refresh + achievements check). Budget header gains an inbox button with a count badge (hidden until a connection exists).
- **`CategoryPillPicker` extracted** from the duplicated pill blocks in Add/EditBudgetEntryModal (behavior-preserving, same `SELECTABLE_BUDGET_CATEGORIES` filter; `pinCurrentValue` reproduces Edit's keep-legacy-category behavior) - now shared by both entry modals, the wizard, and the inbox.
- **`ConnectionsProvider`** context (CustomCategoriesProvider pattern) owns connections/pendingCount/needsAttention/isSyncing for Profile, Budget, and the modals.

### Cross-cutting round-trip safety

- **Validators** (`recordValidators.ts`): `isBudgetEntryItem` + `explainBudgetEntryProblem` (lockstep) accept the three provenance fields with bounds (`source === "bank"`, externalTxId ≤200, merchant ≤120) - one edit covers both the JSON import and LAN-sync receive gates.
- **Spreadsheet round-trip** (`spreadsheetExport.ts`/`spreadsheetImport.ts`): new `Source`/`ExternalTxId`/`Merchant` columns on the Budget Entries sheet. Data integrity, not convenience: an xlsx backup/restore that stripped `externalTxId` would make the next sync re-offer every approved transaction. Round-trip tests extended (xlsx + csv) including a manual-entries-stay-clean assertion.
- **Tests:** new suites cover simplefin/teller parsers, ingest dedup incl. id-instability aliasing, merchant normalization, cooldown/window math, the HTTP/1.1 codec incl. chunked bodies, validator lockstep, export exclusion, and spreadsheet round-trip (the Schwab parser/token-state suites left with the connector). Full suite as of 2026-07-11: 643 passing across 47 suites.

### Follow-ups landed before release (2026-07-08 → 2026-07-11)

- **Security hardening (`cd12b3e`).** SimpleFIN access URLs are https-only (URL-embedded basic-auth credentials can never travel cleartext). `encryptedStorage.setItem` gains a `requireEncryption` option; connection secrets use it, so bank credentials **throw** (`EncryptionUnavailableError`) instead of silently falling back to plaintext when the OS keystore is unavailable, and connect flows preflight `isEncryptionAvailable()` to surface a clear message up front. Also fixed an unhandled-rejection leak in `enqueueWrite`'s cleanup. Tests for both guarantees.
- **In-app setup guide (`da9691b`).** Full-screen provider guide (`src/data/connectionGuides.ts`, editable + unit-tested) reachable from the provider picker and every setup step: verified step-by-step instructions, links to provider sites/docs, cost and gotchas, privacy-at-a-glance, and a "Start setup" shortcut into the wizard. Jest `testTimeout` raised to 15s (real-timer pairing test intermittently timed out under coverage).
- **Teller: add another bank (`74de956`).** A Teller connection can hold multiple enrollments: "+ Add another bank" on the detail view re-opens Teller Connect with the stored app id/cert/environment; only not-yet-linked accounts are offered for mapping (links upsert by external id, so existing mappings never reset). The Teller environment now persists on the secret (defaults to `development` for pre-existing connections). SimpleFIN unaffected - new banks appear automatically on sync.
- **Sync fix: double-counted debt payment (`faa0ad4`).** Prompt-logged minimums carry a deterministic id (debt + month) so the sync merge collapses both phones' confirmations into one record; a launch-time + post-merge repair tombstones existing duplicates, gated on the debt's balance arithmetic so a genuine double payment is never touched.
- **Expo SDK 57 / RN 0.86 / TypeScript 6.0 upgrade (`49a3e15`).** All Expo packages follow the SDK 57 map (`npx expo install --check` clean). eslint stays on 9.x (eslint-plugin-react caps at `^9.7`); `tsconfig` gains explicit `"types": ["jest"]` and `"rootDir": "."` for TS 6. `xlsx` stays on the SheetJS CDN tarball. Crypto-relevant packages (`crypto-js` 4.2.0, `react-native-get-random-values`, async-storage) unchanged - encrypted backups from older versions import unchanged, now locked by golden-fixture tests: hard-coded v1 (legacy KDF) and v2 (PBKDF2) ciphertexts in `importData.test.ts` fail CI if any future dependency change breaks decrypting existing backups.
- **React Compiler compliance.** All 43 `react-hooks` lint warnings (new compiler-backed rules from config-expo 57) refactored away across 23 files, so the compiler optimizes these components instead of skipping them: `useAnimatedValue` replaces `useRef(new Animated.Value()).current`, a shared `useValueChanged` hook (`src/hooks/`) replaces synchronous setState-in-effect resets with render-time guarded adjustments, nav-param modal opens defer past the tab transition via `InteractionManager`, and the debt/entry modals' form-field mapping is single-sourced (`debtFormState`/`entryFormState`). Also fixes a latent coachmark bug (context re-render mid-tour could reset the tour to step 0). Repo lint: 0 warnings.

### Tracking Reminders - opt-in check-in notifications (2026-07-12)

**Adds `expo-notifications@~57.0.3`** (config plugin in `app.json`) - another native dependency riding this release's already-required EAS build. Local notifications only: planned on-device from the user's own data, no push token, no server. Product decision: payment/bill-due push notifications were considered and **rejected** - banks already send those and the in-app due banners cover the surface - so notifications target habit retention instead.

- **Two reminder kinds, each behind its own toggle** (master toggle default OFF, strictly opt-in):
  - *Quiet-spell check-ins* - anchored `cadenceDays` (1/3/7) after the newest budget entry `createdAt` (`updatedAt` deliberately ignored: sync merges aren't tracking). Rescheduled on every app open + background transition, so logging an entry silently pushes every pending nudge out a full cadence; active trackers never get pinged. Already-overdue users get nudged at the next occurrence of the chosen hour rather than a full cadence later.
  - *Month-start planning* - the 1st of each month: set this month's budget goals, review last month (points at the existing budget-limit + Monthly Review surfaces on Budget). A check-in colliding with a month-start day is dropped so no day ever gets two notifications.
- **`src/utils/trackingReminderPlanner.ts`** - pure planner (no expo/RN imports; 20 Jest cases): anchor math, overdue roll-forward, month-start scheduling + same-day dedupe, 30-day window, 32-request cap (iOS keeps the 64 soonest pending), deterministic identifiers (`budgetark-checkin-YYYY-MM-DD`, `budgetark-monthstart-YYYY-MM`) so replans are idempotent, and copy rotation keyed by calendar day/month so a replan never reshuffles messages already in the tray. All copy is content-free (no amounts, no account names) - lock-screen safe by construction.
- **`src/notifications/trackingReminders.ts`** - OS integration: Android channel (`tracking-check-ins`), permission flow (`canAskAgain`-aware; permanent denial surfaces an Open Settings path in the sheet), idempotent cancel-ours-then-reschedule keyed on `content.data.type === "tracking-reminder"`, serialized in-flight guard, never throws (best-effort from boot/background/settings-save callers). Foreground handler suppresses banners while the app is open but keeps the tray/list entry.
- **`src/components/TrackingReminderHost.tsx`** - app-root host: reschedules on launch (deferred past first paint via `InteractionManager` - it decrypts budget entries, same reasoning as `DebtDueReminderHost`) and on every move to background; routes notification taps to the Budget tab, handling both warm taps and the cold-start response (`getLastNotificationResponseAsync`, once per launch, with a navigation-ready retry).
- **Settings** - Profile → Tracking Reminders row → bottom sheet (`TrackingRemindersModal.tsx`, TipJarModal conventions): master toggle runs the permission request on enable; per-kind toggles; cadence chips (daily / every 3 days / weekly); shared time-of-day chips (morning 9 / afternoon 13 / evening 19). Persisted in `trackingReminderSettingsStorage.ts` (EncryptedStorage, sanitized on read) - **per-device, deliberately NOT synced** to a paired partner.
- **Reset All Data** wipes the settings key (added to `RESET_KEYS`) and cancels all pending scheduled notifications immediately.
