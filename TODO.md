...
- [~] In-app donation support (Tip Jar) via Apple/Google billing with privacy-safe wording and no custom payment data storage - App side SHIPPED; store-console product setup still required (below).
  ...
  - Occasional post-win nudge (2026-08-29): `utils/tipJarNudge.ts` cadence (every 4th win, at most weekly; deliberately no off switch), `storage/tipJarNudgeStorage.ts` (device-local, records nothing about actual tips), `tipjar/TipJarProvider.tsx` (root-mounted sheet + floating toast, `useTipJar()`), `TipJarNudgeCard` inline in the payment/payoff celebrations and the Review Inbox, toast after DebtCard payments and bill-linked entry saves. Device testing pending.
...
- [x] Spending Velocity Alerts - passive banner when opening the app: "You've spent 60% of your Grocery budget and it's only the 12th." No push notifications required. DONE 2026-08-28: `utils/budgetPacing.ts` (pure, tested) + `SpendingPaceBanner` on the Budget tab (current month only, over-limit first then ahead-of-pace, tap expands the category) + a pace mark and amber "ahead" colour on every limited category bar in the Spending card. Device testing pending.
...
- [x] Import presets for YNAB / Mint / Monarch (built 2026-09-02, v1.10.1) - `utils/importPresets.ts` detects the export by signature headers and rewrites rows into the Budget Entries schema before the normal validation; transfers dropped and counted; keyword category mapping with custom-category fallback. Documented in the schema modal + `docs/SPREADSHEET_SCHEMA.md`.
  - [ ] Follow-up: verify against real export files from each app (header names drift between app versions); account-name → Bridge account mapping.
...
- [~] **Phase 2 - UI chrome extraction (the long part, several weeks).** Started 2026-09-18: Onboarding + Profile done. 2026-09-21: Budget tab done (933 keys total) - BudgetScreen, BudgetEntryModal, ReviewInboxModal, SpendingCard, BudgetBucketCard, BudgetLimitsModal, FoodSplitModal, GlobalSearchModal (shared with Debts), BillCalendarModal, MonthlyReviewModal, PaycheckCycleCard, MonthBalancePromptModal, CashFlowCard, TrackingReminderOfferCard, DebtDueReminderBanner, DueDateReminderBanner, SpendingPaceBanner, CategoryPillPicker. Category DISPLAY names now go through `src/i18n/categoryLabel.ts` (`categories.<name>` table keyed by the raw BUDGET_CATEGORIES value; storage/sync/compare untouched; custom names pass through). Bucket labels live once in `buckets.<id>`. Still English inside the Budget tab: MerchantRulesModal, AttachmentSection, MonthYearPicker, SheetModal chrome, TipJarNudgeCard; util-produced text (`formatMonthKeyLabel`, `formatYearMonthLabel`, `getRecurrenceTag`, `budgetInsights` streak labels, `reviewInboxSections` titles, `describeUnusualCharge`, `paycheckCycle` labels, `searchFilter` option labels, `customCategoriesStorage` add() errors); "W-2"/"1099"/"401(k)" kept verbatim. Known follow-up: search matches raw category names, so a German user typing "Lebensmittel" won't find "Grocery" - match against `categoryLabel` too. 2026-09-21: Debts tab done - DebtTrackerScreen (strategy + Build Your Ark milestones + undo), AddDebtModal, MonthYearPicker (shared, `datePicker.*`), DebtCard, PaymentHistoryModal, DebtPayoffCelebrationModal, DebtPaymentCelebrationModal, DebtFreeCountdownCard, DebtDuePaymentPromptModal, CardKeepAliveBanner, TipJarNudgeCard. Owner/debt-type labels live once under `debts.form.{owner,type}.options.<id>`. Still English in the Debts tab: milestone step title/description/metricLabel/nextAction from the milestone util (cards say Keel/Hull while the German intro says Kiel/Rumpf - key them by milestone id in the util pass), `describeUnsolvablePayoff`, `tipNudgeCopyFor`, DEBT_OWNER/CLASS_OPTIONS `.label` in src/types (other callers). The Ark savings-entry description ("Logged from Build Your Ark") is now stored in the active language - nothing matches on that text (verified), deliberate. 2026-09-21: Bridge tab done (1454 keys total) - BridgeScreen (accounts card, inline asset + emergency-fund modals, holdings nudge/prices, disclosure buttons), PurchasePlanList, AnnualReportModal, NetWorthHistoryCard, NetWorthProjectionCard, CashFlowChart, TrackingStripCard, plus AchievementsScreen chrome (`achievements.*`). Asset categories / change periods / plan ranking + allocation modes / history ranges / achievement tiers translate by id. Still English on the Bridge: holdings disclosure body (src/data, rule 4 copy), badge titles/descriptions (src/data/achievementDefs.ts), AchievementUnlockModal, and util-produced sentences: `describeApy`/`describeApyGap` (savingsInterest), `formatNextQuoteRefresh` (bridgeMath), `describeDebtOpportunityCost`/`describeCostPerUse` (purchasePlanner), `formatAnnualReportShareText` (annualReport), `describeDaysSince` (trackingStrip), `formatDayLabel` fallback (dateFormat), SparklineChart axis numbers (device locale). `DEFAULT_BROKER_NAME` "My Holdings" deliberately stays English - it is stored data the orphan-holdings migration matches on. 2026-09-22: Charts tab done (1891 keys total) - ChartsScreen (learning hub + tools), TaxCalculatorCard + QuarterlyTaxCard (chrome only, German titles carry "(nur USA)", US terms verbatim, no German tax content), PurchasePlannerCard, LoanCalculatorCard, WhatIfSpendingCard, SubscriptionDetectiveCard, CurrencyExchangeCard, PersonalInflationCard, and the lessons reader chrome (src/lessons/*, `charts.lessons.*`) with an "English only" note under the lesson title when the app language isn't English. Lesson prose, chapter/lesson titles, learning disclaimer, exchange-rate disclosure body stay English (src/data). Util sentences still English: `describeCostPerUse`/`describeHoursOfWork`/`buildArkPurchaseGuidance` (purchasePlanner), `formatWhatIfMonths` (whatIfSpending), plus whatever the subscription/inflation helpers return. 2026-09-22 (later): shared modals done under `modals.{connections,guard,data,people,engage}.*` (2532 keys total) - AddConnectionModal, ConnectionsModal, ProviderSetupGuideModal, TellerConnectModal; PairingModal, AutoBackupModal, AppLockSetupModal, AppLockGate, PinPad, FeedbackModal; MerchantRulesModal, BankStatementImportModal, SpreadsheetSchemaModal, ManageCategoriesModal (TagPillPicker labels passed in); LoansModal, BusinessReportModal, PersonReportModal, ManagePeopleModal, ManageBusinessesModal, AttachmentSection, AttachmentViewerModal; TipJarModal, TrackingRemindersModal, FeatureSpotlightModal, OnboardingGuideModal, AchievementUnlockModal, NewFeatureBadge. No-string components skipped (reminder hosts, SheetModal, Medal, charts, backgrounds). Every src/components file with user-facing text now uses `useTranslation`. Deliberate English: spreadsheet sheet titles + column names (importer matches them), Teller environment ids, feedback email subject tags + diagnostic lines, `[Bug Report]`-style triage text, pairing-code/address format placeholders, stored tip entry description. Still English (util/data/service): `INCORRECT_PIN_MESSAGE` is mapped at render in both lock components (hook untouched); `customCategoriesStorage.add()` errors; PeopleProvider validation errors; `formatDayLabel` in LoansModal; connection service error messages; guide search matches English coachmark text (a German search for "Beleg" finds nothing until the data pass); FEATURE_SPOTLIGHTS / COACHMARKS / achievementDefs / connectionGuides copy. Phase 3 util catalogue (helpers that return English sentences, to take a `t` param with tests on a real i18next instance initialised from the en tree): spreadsheetImport skip reasons + file errors (36), importData rejection messages (26), trackingReminderPlanner notification copy (12, rule 11), spreadsheetExport timeouts (11), debtTrackerMath milestone titles/nextAction/metricLabel (10), purchasePlanner ordering blurbs + describeCostPerUse/describeHoursOfWork/describeDebtOpportunityCost/buildArkPurchaseGuidance, bankCsvImport errors + DEFAULT_STATEMENT_ACCOUNT_LABEL, reviewInboxSections titles, exchangeCalculator "Rates updated", cardKeepAlivePlanner notification copy, budgetInsights labels, tipJarNudge titles, bridgeMath formatNextQuoteRefresh, calculations describeUnsolvablePayoff, whatIfSpending formatWhatIfMonths, unusualCharges, paycheckCycle labels, annualReport share footer, plus formatMonthKeyLabel/formatYearMonthLabel/formatDayLabel (budgetMonths, dateFormat), getRecurrenceTag, describeApy/describeApyGap, describeDaysSince, describeSyncActivity (src/sync), cadenceLabel (autoBackupPlan), searchFilter option labels, currency option labels. 2026-09-22 (later still): phase 3 done under `helpers.{import,notifications,insights,planning,misc}.*` (2809 keys total). Mechanism: plain modules translate via `src/i18n/translate.ts` (global i18next `t` + `currentLanguage()`); Jest initialises the global instance with the English tree in `src/i18n/jestSetup.ts` (setupFiles) so util tests keep asserting English sentences; `src/i18n/__tests__/translate.test.ts` pins that. Module-load label tables became functions (`checkInMessages()`, `duplicatesSectionTitle()`, ...); unused English label tables deleted where callers already translate by id (searchFilter, paycheckCycle presets, purchasePlanner priority/allocation labels, `cadenceLabel`, `PAY_FREQUENCY_LABELS`). Milestone step title/description/nextAction now localized by id in `computeMilestoneProgress` (Kiel, Rumpf, Deck, Vorräte, Tiere sammeln, Anker, Segel). `importData` throws a typed `PasswordRequiredError` (DataSection uses `isPasswordRequiredError`, no more English message matching). `usePinVerifier` translates its own error (the render-time mapping in both lock components is gone). Connection service errors keyed under `helpers.misc.connections.*`. Notification copy keyed by word ids (typed `t` rejects numeric keys); planner tests assert rule 11 in German too. A real language switch calls `rescheduleTrackingReminders()` + `rescheduleCardKeepAliveReminders()` from LanguageProvider so pending notifications re-issue in the new language. Memoized callers of translated helpers list `t` in deps with a justified exhaustive-deps disable (BudgetScreen, ReviewInboxModal, DebtTrackerScreen x2, PurchasePlannerCard, BridgeScreen). Known leftovers: `connection.lastErrorMessage` is persisted in the language active when the sync failed; CurrencyExchangeCard stores the rates-updated label in state until the next fetch; number placeholders like `2.5 weeks` are not locale-formatted in German; `DEFAULT_STATEMENT_ACCOUNT_LABEL` / `DEFAULT_BROKER_NAME` / spreadsheet sheet+column names / `missingSections` sheet names stay English (file + storage contracts). 2026-09-22 (evening): data pass done under `data.{achievements,spotlights,coachmarks,guides,templates,disclosures}.*` - every text field on the bundled tables is a getter resolving through translate.ts (consumers untouched); plain string constants became functions (`learningDisclaimer()`, `connectionsDisclosureTitle()/Intro()/Points()` etc.). Guide search now matches German coachmark text + keywords. Search matches translated category + debt owner/type labels alongside the stored value (`searchFilter.ts`, German test). Widgets: Android Quick Entry renders `categoryLabel` after applying the stored language in the headless handler and is refreshed on a language switch; iOS target follows the DEVICE language with inline German (no App Group by design). Release plumbing: 1.11.0 entry in both release-note files, `german-language` FEATURE_SPOTLIGHT (requiresRuntimeVersion 1.11.0, CTA opens the Language picker via the new `language` profile section + NEW badge on the row), app.json version 1.11.0. Final: 3178 keys, 160 suites / 2467 tests, typecheck 0, lint 1 pre-existing warning.
  - [ ] **Human review before shipping:** the German rule-4 consent copy in `src/i18n/locales/de/dataDisclosures.ts` (`data.disclosures.{connections,exchangeRates,holdings}.*`, plus `learningDisclaimer`) must say exactly what the English says. Also skim `de/helpersNotifications.ts` (rule 11) and the German feedback-email template.
  - [ ] **Device testing (no rig here):** first paint in German on a German phone; Language row switch re-renders every tab, modals, widget, and re-issues reminders; `Intl.PluralRules` on Hermes (dev warning if missing); German overflow on tab labels / chips / segmented controls / the milestone bar; Android widget after switch; iOS widget on a German device; encrypted-backup import password prompt (now `isPasswordRequiredError`); guide search for "Beleg".
  - [ ] Known leftovers: `connection.lastErrorMessage` persisted in the failing sync's language; CurrencyExchangeCard keeps its rates-updated label in state until the next fetch; German number placeholders like `2.5 weeks` not locale-formatted; Android widget `description` in app.json is manifest English; theme names, lessons, US tax content, spreadsheet contracts stay English by design.
  - [x] Russian / Ukrainian done 2026-09-23: `SUPPORTED_LANGUAGES` = en/de/ru/uk, `LANGUAGE_NATIVE_NAMES` Русский / Українська, every fragment has ru/ + uk/ twins typed `LocalizedPlural<typeof en>` (expands each `_one` to `_one/_few/_many`; 3406 keys per language). `locales.test.ts` checks per-language plural forms; `_few`/`_many` may follow either the English `_one` or `_other` placeholder set. `intl-pluralrules` is required conditionally in `src/i18n/index.ts` because i18next falls back to a one/other rule without `Intl.PluralRules` (wrong for Slavic plurals). Informal ты / ти; glossaries in GLOSSARY.md. iOS widget carries inline ru/uk strings. Known: 'bucket' is корзина/кошик in Budget copy and группа/група in the categories modal; "What if" wording varies by surface.
  - [ ] **Human review before shipping (ru/uk):** `src/i18n/locales/{ru,uk}/dataDisclosures.ts` rule-4 copy (`data.disclosures.{connections,exchangeRates,holdings}.*`, `learningDisclaimer`) and the rule-11 notification copy in `helpersNotifications.ts`. Ideally a native speaker skims the tab labels and chips for overflow. -> data files -> search on translated category names -> widgets -> release plumbing. Move every user-facing literal in ~230 files to a key. Mechanical but easy to get subtly wrong; do it screen-by-screen with typecheck + lint + tests green after each. Order: navigation/tab labels → Profile → Onboarding → Budget → Debts → Bridge → Charts → components. Translate chrome only in this phase; lessons and the tax tool stay English with a small "English only" note. This gets a usable app to the requesters at roughly a third of the total cost.
...
### New findings from the Tier 4 test pass (2026-08-28) - all FIXED 2026-08-28 (staged)

- [x] `encryptedStorage.multiSet` (~:737) falls back to plaintext when no vault key is available and has no `requireEncryption` option, so the `EncryptionUnavailableError` docstring (~:425, "thrown by setItem/multiSet when a caller requires encryption") is wrong. Only non-secret callers today (debtStorage payment paths); fix the docstring or add the option before any secret-bearing caller uses it.
- [x] `encryptedStorage.multiSet` (~:734-745) awaits `getEncryptionKey()` BEFORE splicing itself into `writeQueues`, so a `setItem` on the same key issued in that window runs first and the earlier-issued `multiSet` lands last (silent overwrite). Fix: claim the queue tail synchronously before the await. No in-app caller hits it today (PaymentHistoryModal serializes).
- [x] `debtStorage.normalizeDebt` (~:142) keeps `debtClassSource: "manual"` on a record whose class BudgetArk just inferred / split from legacy `car_house` - only an invalid stored source gets rewritten to "inferred".
- [x] `customCategoriesStorage.validateName`: the built-in-category collision check is case-sensitive while the custom-category check is case-insensitive, so a custom "food" can coexist with built-in "Food". Fix: `multiSet(pairs, { requireEncryption })` + queue tail claimed before the first await; `debtClassSource` forced to "inferred" whenever the class was inferred/split; `categoryIcons.collidesWithBuiltInCategory` (case-folded) used by validateName + the sync add path. Tests flipped from pinning to asserting.
