...
- [~] In-app donation support (Tip Jar) via Apple/Google billing with privacy-safe wording and no custom payment data storage - App side SHIPPED; store-console product setup still required (below).
  ...
  - Occasional post-win nudge (2026-08-29): `utils/tipJarNudge.ts` cadence (every 4th win, at most weekly, Profile > Tip Jar On/Off), `storage/tipJarNudgeStorage.ts` (device-local, records nothing about actual tips), `tipjar/TipJarProvider.tsx` (root-mounted sheet + floating toast, `useTipJar()`), `TipJarNudgeCard` inline in the payment/payoff celebrations and the Review Inbox, toast after DebtCard payments and bill-linked entry saves. Device testing pending.
