...
| `bankCsvImport.test.ts` | `bankCsvImport.ts` | Bank statement CSVs: header-row / preamble / headerless detection, column-mapping guesses for common bank layouts, sign-convention guess, split debit/credit, fail-closed dates and amounts, deterministic row ids + ordinals, inbox-capacity selection, planner integration (dedupe, duplicate flags) |
